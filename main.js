...
if (!won) {
  ...
  level.innerHTML += `<div id = "restart" class = "game-buttons" onclick = "Restart();">undo</div><div id = "home" class = "game-buttons" onclick = "ShowMenu();">menu</div><div id = "moves">moves  ${moves}</div>`;
}
}
...
if (!transferring) {
  if (clicked.length == 0) {
    ...
    clicked.push(x);
    let el =
      document.getElementsByClassName("test-tube")[clicked[0]];
...
document.getElementsByClassName("test-tube")[
  a
].innerHTML += `<div id = "transparent-bg" style = "top:calc(10px + ${p[1] * 30
  }px - 1px);"></div>`;
setTimeout(function () {
...
document.getElementsByClassName("test-tube")[
  b
].innerHTML += `<div id = "colorful-bg" style = "background-color:${p[0]
  };top:calc(10px + ${q[1] * 30}px);"></div>`;
setTimeout(function () {
  document.getElementById("colorful-bg").style.height =
    count * 30 + 1 + "px";
  document.getElementById("colorful-bg").style.top = `calc(10px + ${q[1] * 30
    }px - ${count * 30}px)`;
}, 50);
}
...
won = true;
//console.log("hello");
level.innerHTML = `<div id="won"> 🏆 you won </div><div id = "restart" class = "game-buttons" onclick = "Restart();">restart</div><div id = "home" class = "game-buttons" onclick = "ShowMenu();">go to menu</div>`;
}
